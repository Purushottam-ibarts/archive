...
import { filterData, getWellBeing } from '../../store/wellBeing/wellbeingActions'
...
const HealthWellbeig: React.FC = () => {
  const dispatch = useAppDispatch()
  const navigation = useNavigation()
  const user = useAppSelector((state) => state.userSlice);
  const wellBeing: any = useAppSelector((state) => state?.wellbeingSlice?.wellBeing);
  const hashtags: any = useAppSelector((state) => state?.wellbeingSlice?.hashtags);

  useEffect(() => {
    dispatch(getWellBeing())
  }, [])
  ...
  const onSelect = (val: any) => {
    console.log('val: ', val);
    dispatch(filterData({ contentType: 'healthandwellbeing', nameOfHashtag: val}))
  }
